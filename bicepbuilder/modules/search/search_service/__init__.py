...
class SearchService(TypedDict, total=False):
    ...
    roleAssignments: List[Union['RoleAssignment', Literal['Contributor', 'Owner', 'Reader', 'Role Based Access Control Administrator', 'Search Index Data Contributor', 'Search Index Data Reader', 'Search Service Contributor', 'User Access Administrator']]]
    """Array of role assignments to create."""
...
class SearchServiceBicep(Module):
...
def search_service(
    ...
    if depends_on:
        ...
        bicep.write(f"  ]\n")
    bicep.write(f"}}\n")
    output = SearchServiceBicep(symbol)
    output.outputs = {
