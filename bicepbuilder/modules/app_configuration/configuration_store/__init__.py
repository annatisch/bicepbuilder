...
class AppConfigurationStore(TypedDict, total=False):
    ...
    roleAssignments: List[Union['RoleAssignment', Literal['App Compliance Automation Administrator', 'App Compliance Automation Reader', 'App Configuration Data Owner', 'App Configuration Data Reader', 'Contributor', 'Owner', 'Reader', 'Role Based Access Control Administrator', 'User Access Administrator']]]
    """Array of role assignments to create."""
...
class AppConfigurationStoreBicep(Module):
    outputs: AppConfigurationStoreOutputs


def app_configuration_store(
        bicep: IO[str],
    ...
    if depends_on:
        ...
        bicep.write(f"  ]\n")
    bicep.write(f"}}\n")
    output = AppConfigurationStoreBicep(symbol)
    output.outputs = {
