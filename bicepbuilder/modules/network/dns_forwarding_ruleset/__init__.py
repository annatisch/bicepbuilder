...
class NetworkDnsForwardingRuleset(TypedDict, total=False):
    ...
    lock: 'Lock'
    """The lock settings of the service."""
    roleAssignments: List[Union['RoleAssignment', Literal['Contributor', 'DNS Resolver Contributor', 'DNS Zone Contributor', 'Network Contributor', 'Owner', 'Private DNS Zone Contributor', 'Reader', 'Role Based Access Control Administrator']]]
    """Array of role assignments to create."""
...
class NetworkDnsForwardingRulesetBicep(Module):
    outputs: NetworkDnsForwardingRulesetOutputs


def network_dns_forwarding_ruleset(
        bicep: IO[str],
    ...
    if depends_on:
        ...
        bicep.write(f"  ]\n")
    bicep.write(f"}}\n")
    output = NetworkDnsForwardingRulesetBicep(symbol)
    output.outputs = {
