...
class NetworkManager(TypedDict, total=False):
    ...
    lock: 'Lock'
    """The lock settings of the service."""
    roleAssignments: List[Union['RoleAssignment', Literal['Contributor', 'IPAM Pool Contributor', 'LocalNGFirewallAdministrator role', 'Network Contributor', 'Owner', 'Reader', 'Resource Policy Contributor', 'Role Based Access Control Administrator', 'User Access Administrator']]]
    """Array of role assignments to create."""
...
class NetworkManagerBicep(Module):
...
def network_manager(
    ...
    if depends_on:
        ...
        bicep.write(f"  ]\n")
    bicep.write(f"}}\n")
    output = NetworkManagerBicep(symbol)
    output.outputs = {
